...
这部分内容必须严格遵守 YAML 语法规范，并且必须写在 SKILL .md 文件的顶部。YAML Metadata 由三个连字符"---"包裹，在"---"之前不能有空行或任何其它内容，并采用字段名: 值（Key: Value）的结构，其中冒号后的空格是强制性的（例如name: my-skill，而非name:my-skill）。其中名称（name）和描述（description）是必填的两个字段：

> - `name`是技能的唯一标识，必须仅包含小写字母、数字和连字符，长度限制在64个字符以内，其中字段名称"name"必须是小写。其值推荐使用以小写字母开头的英文连字符分隔式动名词形式（如 pdf-processing、data-analysis），且必须与技能所在父目录的文件夹名称严格一致。
> - `description`是模型判断“何时使用该技能”的唯一依据，该字段必须非空且少于1,024个字符，其中字段名称"description"必须是小写。其值需清晰简洁地说明这个 Agent Skills 解决什么问题、在什么情况下使用，并包含关键词义词，以便 Agent 快速判断其与当前任务的匹配度——该字段为元数据中权重最高的必填项。如果 `description` 无法清楚区分它与其他 Agent Skills，说明职责边界尚未定义清楚。推荐使用第三人称精确描述能力和触发场景（例如“此技能专门用于 PDF 格式文档的内容读取”）。
...
在 YAML Metadata 中，仅有 name 和 description 参与系统的路由决策。因此，只要准确定义了这两个字段，技能即可被 Claude Code 正常索引与加载。

除了`name`和`description`以外，还可以加入一些其他可选的字段，开发者可以根据需要添加自定义元素数据（如 `license`、 `author`、 `version` 等），Claude 官方在语法校验层面对此类扩展持开放态度。自定义元数据的添加不会对技能的路由决策产生任何实质性影响，同时也不会影响技能的可用性。
...
这一部分定义的是 Agent Skills 的**行为方式**，而不是输出格式，在执行指令中需要明确：
...
Markdown 正文与用户的对话历史、系统提示词共享同一个上下文窗口，因此，需要遵守上下文“经济学”的原则：

> 默认假设——Claude Code 已经具备丰富的知识储备。无须解释“什么是 Python”或“什么是 PDF”等基础概念，直接告知其使用何种库、执行何种操作即可。

Claude Code 建议将 `SKILL.md`的主体内容控制在 500 行以内。如果内容过多，应拆分为外部参考文件。需要强调的是，这部分内容仅在技能被触发时才会消耗 token （通常在 5 token 以内），但一旦加载，就会持续占用宝贵的推理资源，因此精简要义至关重要。

##### 清晰

如果给出的指令过于宽泛，AI 无法明确具体操作（如去重、格式化、处理缺失值）及质量标准，容易产生不一致的结果。

通过提供清晰的示例，AI能够理解具体的操作模式（例如识别并转换各种日期格式），从而对新输入做出符合预期的、一致的格式化处理。

#### 正文指令的设计原则

为了突破单个提示上下文长度（如 500 行）的限制，同时保持主文档的清晰与可读性，建议将 Markdown 正文指令设计为“目录索引”而非“内容容器”。

具体而言，主文件 `SKILL.md`应聚焦于：

> - **高层级的任务描述**
> - **执行流程概念**
> - **关键规则说明**

而将详细实现逻辑、复杂配置、示例数据或领域知识拆分到独立的外部文件中。例如，正则表达式规则可存入 config/extraction_rules.json，医学标准定义置于 standards/diabetes.md，具体脚本代码保留在 scripts/normalize_diagnosis.py 等。主 Markdown 通过清晰的路径引用（如“详见 scripts/extract_metrics.py”或“参考 standards/vital_ranges.md”）引导 AI 按需加载相关内容。

`YAML 前置元数据`和 `Markdown 正文指令`共同构成 `SKILL.md` 的整体内容结构，成为 Agent 完成特定任务的技能指南。

### 资源文件的类型、组织与引用

为保持 `SKILL.md` 的精简与高性能，需将详细信息剥离到外部文件中，构建一个类似“图书馆 + 工具箱”的协同结构。该设计遵循渐进式披露原则，`SKILL.md` 仅作为目录索引，引导 Agent 在必要时查阅特定文件或使用特定工具。

在为资源文件命名时，需注意文件名称应清晰反映其内容（如 form_validation_rules.md），避免使用 `doc1.md`、`file2.txt` 等无意义的命名，含有清晰语义的文件名有助于模型在浏览目录时准确判断是否需要读取该文件。
...
这些资源相当于按需加载的“图书馆”，通常是一些 markdown 文件，它们构成技能的静态知识库。与系统提示词中强制加载的内容不同，这些文件静置在硬盘上，不占用任何上下文窗口，直到任务执行过程中需要查阅时才被加载。

以下是一些关于指令类资源的示例：

- `FORMS.md`——复杂的表单填写指南或字段映射规则
- `REFERENCE.md`——冗长的 API 参数列表、数据库 Schema 定义或错误代码对照表
- `DESIGN.md`——企业或品牌的 VI 设计规范、前端界面的样式风格定义

##### 执行类资源

这是 Skills 架构中最具颠覆性的优势。与现场生成不可控代码的方式不同，开发者可以预置经过严格测试的 Python 或 Bash 脚本，由 Agent 在需要时执行。这种设计带来如下好处：

- 【核心优势】：零上下文执行——当指示 Claude Code 运行 python scripts/validate.py 时，脚本代码完全不需要加载到上下文窗口中。系统仅触发脚本运行，并接收输出结果。脚本本身的代码逻辑不占用任何 token 资源。
- 【设计哲学】：解决问题，别踢皮球（Slove! Don't Punt!）——不要让 Claude Code 尝试“写代码来解析这个 PDF”，而是直接提供一个经过验证的 scripts/parse_pdf.py 并运行。这种设计实现了确定性执行，避免模型生成代码时因模型幻觉而产生错误。

 例如，在执行高风险操作前，需要检查 JSON 配置文件是否合法，可以在 scripts 文件夹下放置一个写好的验证脚本 `validate.py`。

##### 模板类资源

这些资源相当于标准化的“模具”。典型的用法如下：

- 包括“少样本学习（Few-Shot）”的输入/输出对——如果示例过多，将其移出 `SKILL.md` 可以避免干扰模型的注意力，例如提供一个单独的 `EXAMPLES.md`文件。
- 预定义的报告结构或代码脚手架——通过模板强制 Claude Code 输出符合团队统一格式的内容，例如提供一个单独的 `TEMPLATES.md`文件。
...
<div align=center>推荐的技能文件夹结构</div>
...
对于包含大量知识的复杂技能，严禁将所有信息堆砌到单个文件中，应按功能域将资源文件存放在子级目录中，推荐的资源目录包括 `scripts`、`references` 和 `assets`，分别用来放置脚本文件、各类参考文件和静态资源文件。

这些资源目录与文件作为功能支撑要素，与 `SKILL.md` 共同构成一个完整的技能包，从而让技能既具备复杂场景的适配能力，又保持了核心逻辑的简洁与高效。

#### 资源文件的引用

Claude Code 的执行环境基于 Linux 容器。无论开发环境是 Windows 还是 macOS，开发者在编写引用路径时，必须始终使用 Unix 风格路径（不同文件夹层级之间使用"/"进行分隔，例如 scripts/helper.py），严禁使用 Windows 风格路径（即以反斜杠"`\`"分割文件夹的层级），否则将导致文件无法找到。

>[!warning] 防止资源文件嵌套引用
>为了节省 token，Claude Code 在处理深层嵌套的文件引用时，可能会尝试仅读取文件的前几行（类似于 `head-100`）进行预览，导致产生部分读取（Partial Read）现象，从而丢失关键信息。扁平化的目录结构能够确保模型在需要时完整地读取目标文件。
>因此，为了让 `SKILL.md` 在必要时以最短路径读取到资源文件，尤其是在引用参考类文档时，应避免出现资源文件嵌套引用的情况。
>例如不应在参考文件 A 中引入参考文件 B，再将 A 整体引入 SKILL.md，这样会导致 Agent 在读取内容时，由于嵌套太深而漏掉部分信息。所以尽可能直接在 SKILL.md 中引入参考文件，每个参考文件定义单一场景功能。

#### 脚本文件的可执行权限

建议将所有 Python 或 Bash 脚本放入 `scripts/`文件夹下，以保持根目录整洁，
同时确保脚本具有可执行权限：
...
虽然 Python 脚本可以存放在根目录，但标准做法是存放在 `scripts/` 子级目录中。这有助于 Claude Code 区分“文档”和“代码”，并确保脚本具有执行权限。

### 撰写 `SKILL.md` 的最佳实践

一个好的技能创建，通常遵循以下原则：

- 【保持专注】——每个 Skill 只做一件事，不要建立万金油 Skill，而是分别为不同的领域或情境创建独立的 Skills；
- 【清晰准确的`description`】——具体说明它的功能和适应情境，帮助 Agent 识别触发时机；
- 【将`scripts`视为黑箱】——要求 Agent 首先阅读`scripts/generate.sh --help`以了解脚本的用法，而不是直接阅读整个源码，这有助于将 Agent 的上下文保持专注在任务上；
- 【包含决策树】——对于复杂的 Skill ，加入不同`--mode`帮助 Agent 根据情境选择正确的方法；
- 【提供模板示例】——让 Agent 在执行任务时，有参考依据；
- 【使用 Agent 辅助开发】——让 Agent 帮你建立和优化 Skill。

通过 Agent Skills，你可以不断扩展 Agent 的能力，让它成为真正符合你需求的 AI 开发伙伴。

通常你不需要手动撰写所有内容：

> 让 Agent 帮你开发 Skill，然后根据使用经验持续迭代改进。
...
---
## Agent Skills 的触发与检索机制

在渐进式加载架构中，Claude Code 并不具备“全知视角”。它必须仅凭极少的元数据线索（Metadata，即根据 `name` 和 `description`字段的内容），在毫秒级时间内从潜在的数十个技能中作出精准的决策，这一过程分为三个解密衔接的步骤：
...
在对话开始或 Agent 启动时，系统会将所有可用技能的 `name` 和 `description` 字段提取出来，以紧凑的文本形式注入 Claude Code 的系统提示词中。此时，大模型并不知晓 `SKILL.md` 中的具体指令，它只有一份轻量级的“能力菜单”。

当用户发起请求时（如“提取此 PDF 中的文本”），Claude Code 的推理引擎会实时分析用户意图，并将其与“能力菜单”中的描述进行语义比对。

由于 `description` 是大模型判断相关性的唯一凭据，因此该字段必须包含明确的“触发词”（Trigger Terms）和清晰的能力边界（Ability Boundary）：

> 举一个反例，当描述为“处理文件”时过于模糊，大模型无法判断是否适用于 PDF。正确的描述为“从 PDF 文件中提取文本和表格，填写表单…当用户提到 PDF 或文档提取时使用此技能”，这样的模式能确保技能精准触发。

### 主动查阅与上下文置换

Agent Skills 一旦匹配成功，Claude Code 不会等待用户进一步的指令，而是自主决定采取行动。这一机制与传统的斜杠命令（如 `/deploy`）存在本质区别：

- 斜杠命令依赖用户显式调用
- Agent Skills 由大模型根据上下文自动触发

Claude Code 会生成一个具体的 Bash 工具调用命令，通常是 `read` 命令。命令执行后，`SKILL.md` 的内容被读取并作为 Tool Output 返回给大模型，此时技能的完整指令正式进入 Agent
的上下文窗口，大模型开始遵循其中定义的步骤与规范执行任务。

### 冲突消歧

当多个技能的描述存在重叠时（例如两个技能都提到“数据分析”），Claude Code 可能会产生困惑或随机选择，导致路由决策的不确定性。

为了避免“路由冲突”，不同技能的描述必须通过具体的领域限定进行物理隔离：
...
如果存在同名技能，系统还将遵循：
 ...
 ```plain
 Managed > Personal > Project
 ``` 
 
 这种优先级覆盖规则来决定最终加载技能的哪一个版本。
...
大模型在处理开放式创作任务（如诗歌生成）时可产出绝美语句，但在执行结构化、多步骤的确定性任务时，可能出现注意力偏移、步骤遗漏或过度自由发挥等问题。为了将 AI 的行为模式从“自由发挥”转变为“可预期的执行”，需要引入流程约束机制。

### 步骤化工作流与检查点

想象一下，教新手朋友做一道复杂的菜，如果只给出指令“做个佛跳墙”，对方肯定会手忙脚乱。
但如果为其提供一份如下所示的任务清单，思路便一目了然。
...
这就是检查清单的魅力！在 Skills 设计中，要让 AI 养成同样的习惯。具体的设计技巧如下所示。

**(1) 进度可视化**。要求 Claude Code 在执行任务前先显式列出执行步骤清单，并在每完成一步后在对话中标记该步骤为已完成状态（用 TaskList 语法的`- [x] 事项` 表示），使用户能够实时掌握任务进展，示例如下：

```markdown
正在按以下步骤进行代码重构：
- [x] 步骤 1：运行静态分析，找出所有需要重构的函数
- [x] 步骤 2：为每个函数生成重构方案
- [ ] 步骤 3：由人类审核方案
- [ ] 步骤 4：实施通过审核的重构
```

**(2) 设置安全关卡**。当 AI 要进行危险操作时（比如删除文件、修改生产数据库、批量数据更新），必须在执行前引入人工确认机制，形成“预检 - 展示 - 确认 - 执行”的安全闭环。

举一个现实中常见的案例。某团队让 AI 清理临时文件，由于缺乏安全关卡，AI 险些误删整个日志目录。改进后的方案要求 AI 在执行删除操作前先输出“待删除文件列表”，运行检查脚本确认列表中不包含重要文件，并将列表呈现给用户确认，待收到明确肯定结果后才执行删除。这就像装修师傅在砸墙前，一定会用马克笔画线让业主确认：“是砸这里吗？”待业主确认无误后才会动工。

### 输出格式规范：表格、JSON、文档结构

在实际应用中，常常遇到 AI 分析数据时输出格式不一致的问题——有时采用段落描述，有时使用列表，甚至自行生成非标准格式，导致后续必需大量人工整理。

解决办法很简单：避免模型“自由发挥”，采用结构化输出约束为 AI 提供严格的 JSON、表格或文档结构模板，要求其严格按照预设格式输出，示例如下：

`````markdown
请严格按照这个格式输出用户反馈分析：
```json
{
  "总体情绪": "positive/neutral/negative",
  "高频关键词": ["词1", "词2", "词3"],
  "紧急问题": [
    {"问题描述": "...", "建议处理方式": "..."}
  ]
...
大模型存在一个固有倾向——在信息不足时倾向于“自信地猜测”，而非承认未知。这种“不懂装懂”的行为可能导致大模型一本正经地输出看似合理实则无据的分析结果，严重损害系统的可靠性。

#### 核心原则：不要让 AI 瞎猜

能用代码算的，绝不依赖 AI 的推理，将确定性任务从模型的黑盒中剥离，交由可验证的脚本执行。示例如下：

* **反面示例：**
  
```plain
用户向 AI 发送提示：“请检查这个配置文件格式是否正确”。
AI 可能回答：“看起来格式正确，YAML 缩进似乎没问题...”。
其实漏了个冒号。
```
...
```plain
用户向 AI 发送提示：“请运行 `validate_config.py` 来验证配置文件”。
脚本运行后输出：`"ERROR: 第 23 行缺少冒号"`。
```

采用上述方式后，大模型仅负责“调用”与“解读”，而非“计算”与“猜测”，有利于从根本上消除数值幻觉的产生土壤。

#### 设计“防呆”错误提示

当脚本运行发生异常时，应向模型返回结构化的错误信息，明确问题定位与修复建议，而非仅抛出技术性异常。
...
* **好的错误提示：
  ...
  ```python
  print("❌ 输入文件格式不支持。请确认：")
  print("  1. 文件是否为 .xlsx 或 .csv 格式？")
  print("  2. 如果是 .csv，请检查是否用逗号分隔？")
  print("  3. 文件是否被其他程序占用？")
  ```

该设计思路可类比导航 App：

> 当用户偏离路线时，导航 App 不会仅提示：“您已偏离”，
> 而是给出可操作的修正指令：“请在下一个路口掉头”。

#### 建立“错了就改”的循环

为应对模型可能产生的逻辑偏差或输出质量不足，应构建自动验证与迭代修正的闭环流程，使模型能够在反馈驱动下持续改进输出，示例如下：

```text
开始：AI 提交代码
     │
     ▼
运行测试脚本 ────── 失败 ──────► 输出具体哪项测试失败
     │                                 │
     ▼                                 ▼
   成功                           AI 根据错误提示修改
     │                                 │
...
> 某开发团队要求 AI 编写单元测试。
> 初期 AI 编写的测试用例频繁遗漏边界情况，覆盖率仅为 60%。
> 团队设计了一个验证循环：AI 生成测试用例 → 系统运行覆盖率检查 → 如果覆盖率不足，自动指出未被覆盖的函数或程序分支 → AI 根据反馈补充测试。
> 经过三轮迭代后，测试覆盖率从 60% 提升到了 95%。

>[!info] 黄金法则
>黄金法则是指将 AI 当作富有才华但需要详细指导的实习生，指令的结构越清晰，AI 完成任务的确定性越高，设置的检查点越充分，它偏离预期路线的概率越低。
